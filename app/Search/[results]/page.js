...
const SearchResults = ({params}) => {
  ...
  const fetchApiHandler = async () => {
    const resp = await fetch(`https://newsapi.org/v2/everything?q=${querry ? querry : 'bitcoin'}&apiKey=b86b8c1f9ec0423cb814bb0e7a4dfbb5`)
  ...
  return (
    <div className='ResultsWrapper'>
        <div className='searchKeyWord'>You search for <strong>' {querry} '</strong></div>
        {
          cards.length > 0
          ?
          cards
          :
          <h1 className='errorMsg'>'Your Search not found'</h1>
        }
    </div>
  )
}
